...
cpHastySpaceStep(cpSpace *space, cpFloat dt)
{
	...
	cpSpaceLock(space); {
		...
		for(int i=0; i<bodies->num; i++){
			...
			if (body->position_data_func != NULL) {
				body->position_data_func(body, dt, body->position_data);
			} else {
	...
	cpSpaceLock(space); {
		...
		for(int i=0; i<bodies->num; i++){
			cpBody *body = (cpBody *)bodies->arr[i];
			if (body->velocity_data_func != NULL) {
				body->velocity_data_func(body, gravity, damping, dt, body->velocity_data);
			} else {
